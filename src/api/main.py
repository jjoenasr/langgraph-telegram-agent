...
async def lifespan(app: FastAPI):
    async with AsyncSqliteSaver.from_conn_string("test.db") as checkpointer:
        await agent.initialize_agent(checkpointer)
        yield
...
async def webhook(request: Request):
    ...
    if "message" in update:
        ...
        try:
            ...
            await bot.send_message(chat_id=chat_id, text="An error occurred while processing your request. Please try again later.")
    return {"status": "ok"}
...
async def chat(request: ChatRequest):
    ...
    # print([msg.content for msg in await agent.load_prev_messages("test_chat_id")])
